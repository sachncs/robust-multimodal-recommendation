...
def small_config(**recommendation: Any) -> Config:
...
def test_run_writes_the_expected_artifacts(tmp_path: Path) -> None:
...
def test_it_trains_the_ranker_not_the_completion_stage(tmp_path: Path) -> None:
    RecommendationExperiment(config=small_config(), run_dir=tmp_path, items=20, users=8).run()
...
def test_reported_loss_is_a_real_number(tmp_path: Path) -> None:
...
def test_epochs_come_from_the_config(tmp_path: Path) -> None:
    experiment = RecommendationExperiment(config=small_config(epochs=5), run_dir=tmp_path)
    assert experiment.resolved_epochs() == 5
    assert (
        RecommendationExperiment(
            config=small_config(epochs=5), run_dir=tmp_path, epochs=1
        ).resolved_epochs()
        == 1
    )


def test_hyperparameters_reach_the_trainer(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = small_config(lr=4e-4, weight_decay=2e-6, negatives=3, grad_clip=1.5)
...
def test_recommender_kind_is_honoured(tmp_path: Path) -> None:
...
def test_cli_dispatches_to_the_recommendation_service(
