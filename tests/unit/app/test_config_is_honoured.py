...
def small_config(**completion: Any) -> Config:
...
def test_completion_batch_size_comes_from_the_config(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured: dict[str, Any] = {}
    import morel.app.experiment as module
...
def test_manifest_records_the_epochs_actually_trained(tmp_path: Path) -> None:
...
def test_written_config_round_trips_to_the_recorded_hash(tmp_path: Path) -> None:
    """The config written beside a run must be the one its hash was taken from."""
    config = small_config(lr=7e-4, epochs=2)
