...
def test_build_writes_under_the_configured_processed_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.chdir(tmp_path)
    path = write_config(tmp_path, data={"processed": "custom-out"})
    assert main(["build", "--config", str(path)]) == 0
    assert (tmp_path / "custom-out" / "bipartite.npz").exists()


def test_build_applies_the_configured_kcore(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """data.min is the k-core threshold and was never applied."""
    monkeypatch.chdir(tmp_path)
    path = write_config(tmp_path, data={"processed": "out", "min": 4})
    assert main(["build", "--config", str(path)]) == 0
...
def test_kcore_threshold_actually_filters(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from morel.data.store import load_graph

    monkeypatch.chdir(tmp_path)
    loose = write_config(tmp_path, data={"processed": "loose", "min": 0})
    ...
    strict = write_config(tmp_path / "s", data={"processed": "strict", "min": 30})
...
def test_explicit_flag_overrides_the_config(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.chdir(tmp_path)
    path = write_config(tmp_path, data={"processed": "from-config"})
...
def test_mask_uses_the_configured_ratio(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    sparse = write_config(tmp_path, masking={"ratio": 0.1})
    assert (
        main(
            [
                "mask",
    ...
    dense = write_config(tmp_path / "d", masking={"ratio": 0.9})
    assert (
        main(
            [
                "mask",
...
def test_mask_uses_the_configured_seed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    one = write_config(tmp_path, masking={"seed": 1})
    two = write_config(tmp_path / "t", masking={"seed": 2})
    assert (
...
def test_mask_kind_is_selectable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    path = write_config(tmp_path, masking={"kind": "block", "ratio": 0.5})
    assert (
        main(
...
def test_download_category_defaults_to_the_config(tmp_path: Path) -> None:
