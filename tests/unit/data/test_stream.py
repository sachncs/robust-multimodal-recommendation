...
def test_review_stream_chunks_records(tmp_path: Path) -> None:
    records = [
...
def test_review_stream_missing_file_raises(tmp_path: Path) -> None:
...
def test_exact_two_pass_interactions_matches_in_memory(tmp_path: Path) -> None:
    records = [
        {"reviewerID": "u1", "asin": "i1"},
        {"reviewerID": "u1", "asin": "i2"},
...
def test_streaming_item_cooccurrence_construction() -> None:
    chunks = iter(
        [
...
def test_streaming_interactions_emits_pairs(tmp_path: Path) -> None:
    records = [
        {"reviewerID": "u1", "asin": "i1"},
        {"reviewerID": "u1", "asin": "i2"},
        {"reviewerID": "u1", "asin": "i3"},
