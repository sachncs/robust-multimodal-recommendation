...
class Checker:
    ...
    def mismatch(tmp_path: Path) -> None:
        model = nn.Linear(2, 1)
        state = State(
    ...
    def none(tmp_path: Path) -> None:
        mon = Monitor(tmp_path)
        assert mon.latest() is None

    def payload(tmp_path: Path) -> None:
    ...
    def keys(tmp_path: Path) -> None:
        bad_path = tmp_path / "bad.pt"
        torch.save({"model": {}, "unexpected": True}, bad_path)
        with pytest.raises(ModelError):
            safe_load(bad_path)

    def exploit(tmp_path: Path) -> None:
