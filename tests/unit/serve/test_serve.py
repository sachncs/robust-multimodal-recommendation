...
async def get_json(client: httpx.AsyncClient, path: str, **kwargs) -> httpx.Response:
    """Issue a GET and return the response."""
    return await client.get(path, **kwargs)


async def post_json(client: httpx.AsyncClient, path: str, **kwargs) -> httpx.Response:
...
async def test_health_endpoint() -> None:
    async with make_client(create()) as client:
        r = await get_json(client, "/health")
...
async def test_metrics_endpoint() -> None:
    async with make_client(create()) as client:
        r = await get_json(client, "/metrics")
    assert r.status_code == 200


async def test_complete_endpoint() -> None:
    async with make_client(create()) as client:
        r = await post_json(client, "/v1/complete", json={"items": [0, 1, 2]})
    assert r.status_code == 200
...
async def test_recommend_endpoint() -> None:
    async with make_client(create()) as client:
        r = await post_json(client, "/v1/recommend", json={"user": 0, "top": 3})
    assert r.status_code == 200
...
async def test_auth_when_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with make_client(create()) as client:
        r = await post_json(client, "/v1/complete", json={"items": [0]})
