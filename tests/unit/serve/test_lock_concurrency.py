...
def test_multiple_readers_hold_the_lock_at_once() -> None:
...
def test_writer_is_not_starved_by_continuous_readers() -> None:
...
def test_a_waiting_writer_holds_off_new_readers() -> None:
...
def test_writers_are_serialised() -> None:
...
def test_counters_return_to_zero_under_mixed_load() -> None:
...
def test_readers_recover_after_a_writer_times_out() -> None:
...
def test_read_timeout_returns_false_without_acquiring() -> None:
    lock = RWLock()
    lock.acquire_write()
    try:
...
def test_write_timeout_returns_false_without_acquiring() -> None:
    lock = RWLock()
    lock.acquire_read()
    try:
        assert lock.acquire_write(timeout=0.05) is False
...
def test_release_read_without_holding_is_rejected() -> None:
...
def test_release_write_without_holding_is_rejected() -> None:
