...
def test_precision_at_k_shape() -> None:
...
def test_map_at_k_in_range() -> None:
    labels = np.eye(3, 5, dtype=np.float32)
...
def test_per_modality_mse() -> None:
    a = np.array([[1.0, 2.0]])
    b = np.array([[0.0, 0.0]])
