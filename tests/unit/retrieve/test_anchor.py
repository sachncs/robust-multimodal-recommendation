...
def test_query_excludes_self(setup) -> None:
...
def test_query_unknown_modality(setup) -> None:
    features, mask, _ = setup
    assert query(0, "missing", features, mask, top=2).tolist() == []


def test_query_out_of_range(setup) -> None:
    features, mask, _ = setup
    with pytest.raises(GraphError):
        query(99, "visual", features, mask, top=2)


def test_query_invalid_top(setup) -> None:
    features, mask, _ = setup
    with pytest.raises(GraphError):
        query(0, "visual", features, mask, top=0)


def test_batch_returns_list(setup) -> None:
    features, mask, _ = setup
    out = batch([0, 1, 2], "visual", features, mask, top=2)
    assert len(out) == 3


def test_query_skips_zero_norm() -> None:
...
def test_query_shape_mismatch_mask() -> None:
